...
{
  currentrng = RNG_MersenneTwister;
}
void UseRngStream()
{
...
} // The use of c_str is required there because RngStream is a C library using char*, and will convert the C++ string to
  // a char*.
...
double Exp(double lambda)
{
  switch (currentrng) {
    case RNG_MersenneTwister: {
      ...
      return expo(mt19937_rng);
    } // A bracket is necessary because we define expo for the sole purpose of Mersenne-Twister.
    case RNG_RngStream:
